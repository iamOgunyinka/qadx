...
container_id="$(docker container ls --filter 'ancestor=qadx-image' --format '{{.Names}}' | head -n 1)"

if [ -n "$container_id" ]; then
  docker container attach "$container_id"
else
  docker buildx build --platform linux/arm64/v8 -t qadx-image -f "docker/Dockerfile" .
